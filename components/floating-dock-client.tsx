...
export function FloatingDockClient({ navItems }: FloatingDockClientProps) {
    ...
    const links: DockLink[] = [
        ...navItems.map((item) => ({
            title: item.title || "",
            href: item.href || "#",
            // Changed default from "IconHome" to "Home" for Lucide naming convention
            icon: <DynamicIcon iconName={item.icon || "Home"} />,
...
function DockIcon({
    ...
    const Tooltip = ({ direction }: { direction: "vertical" | "horizontal" }) => {
        ...
        return (
            <div
                className={`absolute px-3 py-1.5 ${isHorizontal ? "rounded-xl" : "rounded-lg"
                    } bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/40 dark:border-white/20 ${isHorizontal ? "text-xs md:text-sm" : "text-sm"
                    } font-medium text-neutral-800 dark:text-neutral-200 whitespace-nowrap opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 pointer-events-none shadow-[0_8px_32px_0_rgba(0,0,0,0.2)] ${isHorizontal
